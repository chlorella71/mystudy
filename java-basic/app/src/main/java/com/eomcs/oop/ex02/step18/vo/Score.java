...
public class Score { // => 다른 패키지에서도 이 클래스를 사용할 수 있도록 하려면 public으로 공개해야한다.

  // 인스턴스 변수(instance variable; instanc field)
  // - new 명령으로 생성되는 변수이다.
  // - 성적 데이터를 개별적으로 다루고 싶을 때 인스턴스 변수로 선언한다.(데이터 타입으로서)
  public String name; // => 인스턴스 변수로서 new 명령으로 생성되는 변수
  public int kor;
  public int eng;
  public int math;
  public int sum;
  public float aver; // => 패키지를 분류하였기(유지보수하기 좋게)때문에 다른 패키지에서 사용할 수 있도록 공개해야함 앞에 public을 붙여야함
  // 다른 패키지에서 이 설계도에 따라 만든 변수에 접근할 수 있도록 하려면 public으로 공개
  ...
  // this를 사용한다.
  public void compute() { // => 다른 패키지에서 이 메서드를 사용할 수 있도록 하려면 public으로 공개
  ...
  // 메소드 이름을 클래스이름과 같게 한다.
  public Score(String name, int kor, int eng, int math) {
    this.name = name; // =>this에 저장된 변수에 파라미터값을 넣으라는 뜻
    // name = n;이렇게 하면 컴파일러가 자동으로 this를 붙임. 지금은 String name이 있기 때문에 this를 생략하고 name을 쓰면 중복되어 생략불가함.
    this.kor = kor;
    this.eng = eng;
    this.math = math;
