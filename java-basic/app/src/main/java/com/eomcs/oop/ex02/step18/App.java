package com.eomcs.oop.ex02.step18; // exam0150
...
public class App {
  public static void main(String[] args) {
    ...
    /*
     * Score s1 = new Score(); // => heap이라는 메모리에 생성, 인스턴스 3개(홍길동, 유관순, 임꺽정) 레퍼런스 3개 만들어짐 Score s2 =
     * new Score(); Score s3 = new Score();
     */

    // 1) Score 설계도(클래스 정의)에 따라 인스턴스 변수를 Heap 영역에 만든다.
    // 2) 생성자를 호출한다.
    // - 생성자에 넘겨주는 아규먼트의 타입과 개수, 순서에 따라 호출할 생성자를 결정한다.
    // (파라미터를 받지않는 생성자는 없다) new Score() 빈칸으로 두면 오류뜸
     ...
     * s1.kor = 100; s1.eng = 90; s1.math = 85;
     */
    ...
    /*
     * s2.name = "임꺽정"; s2.kor = 90; s2.eng = 80; s2.math = 75;
     */
    ...
    /*
     * s3.name = "유관순"; s3.kor = 80; s3.eng = 70; s3.math = 65;
     */
    // s3.sum = s3.kor + s3.eng + s3.math;
    // s3.aver = (float) s3.sum / 3;
    // s3.compute(); =>Score 클래스에서 compute값을 ?하면 App에서 호출할 필요가 없다???
    // 그러나 변수의 값이 바뀌면 명시적으로 compute()를 호출하여 합계와 평균을 다시 계산한다.
    // ^^^^^^
